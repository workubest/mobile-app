  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const metricsResponse = await apiService.getDashboardData();
        ...
        try {
          const complaintsResponse = await apiService.getComplaints({ limit: 5 });
          if (complaintsResponse.success) {
            const complaintsData = Array.isArray(complaintsResponse.data) 
              ? complaintsResponse.data 
              : complaintsResponse.data?.complaints || [];
            setRecentComplaints(complaintsData.slice(0, 5));
          }
        } catch (complaintsError) {
          console.warn('Failed to fetch recent complaints:', complaintsError);
          // Don't fail the whole dashboard if complaints fail
        }
